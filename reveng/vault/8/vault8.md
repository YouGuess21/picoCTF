...
We again get a source code with a checkPassword() that looks like a mess, so with help of online beautify tool it looks like:
```
...
```
Here , after password is input its binary bits are scrambled using scramble() and result is checked with expected. So we descramble and get password we need to scramble expected in reverse order for which code will be:
```
...
import javax.crypto.spec.SecretKeySpec;
import java.security.*;
class flag{public static void main(String args[]) {
...
char[] a = password.toCharArray();
 for (int b=0; b<a.length; b++) {char c = a[b];
 c = switchBits(c,6,7);
 c = switchBits(c,2,5);
 c = switchBits(c,3,4);
 c = switchBits(c,0,1); /* d = switchBits(d, 4, 5); e = switchBits(e, 5, 6); */
...
```
Required flag : picoCTF{s0m3_m0r3_b1t_sh1fTiNg_89eb3994e}
